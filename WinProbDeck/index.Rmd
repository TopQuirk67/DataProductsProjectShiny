...
So let's see how the model performs.  First we set in a game situation with a big lead (diff=21) at mid-field, first down and 10 with three minutes left in the game.  We find WP=`r load(file="../pbpdata.RData");
down<-1; togo<-10; ydline<-50; diff<-21; regtime<-180;
df_input<-data.frame(down=factor(down,levels=c("1","2","3","4")),
                             togo=togo,ydline=ydline,diff=diff,regtime=regtime);
round(predict(fit,df_input,type="response"),3)`, a very high liklihood of winning, which makes sense.

Now let's try the other example we suggested, just after kickoff, so diff=0, 80 yards to the endzone, first down and 10, and 3650 seconds in the game.  We get WP=`r load(file="../pbpdata.RData");
down<-1; togo<-10; ydline<-80; diff<-0; regtime<-3650;
df_input<-data.frame(down=factor(down,levels=c("1","2","3","4")),
                             togo=togo,ydline=ydline,diff=diff,regtime=regtime);
round(predict(fit,df_input,type="response"),3)`, very close to the 50% we expect.
...
`r round(predict(fit,data.frame(down=factor(2,levels=c("1","2","3","4")),                           togo=1,ydline=1,diff=-4,regtime=26),type="response"),3)`, while 
